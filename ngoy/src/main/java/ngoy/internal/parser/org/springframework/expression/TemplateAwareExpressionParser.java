...
/**
 * An expression parser that understands templates. It can be subclassed by
 * expression parsers that do not offer first class support for templating.
 *
 * @author Keith Donald
...
public abstract class TemplateAwareExpressionParser implements ExpressionParser {
	 ...
	 * "${...}" markers. For instance: "foo${expr0}bar${expr1}". The static pieces
	 * of text will also be returned as Expressions that just return that static
	 * piece of text. As a result, evaluating all returned expressions and
	 * concatenating the results produces the complete evaluated string. Unwrapping
	 * is only done of the outermost delimiters found, so the string 'hello
	 * ${foo${abc}}' would break into the pieces 'hello ' and 'foo${abc}'. This
	 * means that expression languages that used ${..} as part of their
	 ...
	 * brackets '[' and curly brackets '}' must be in pairs within the expression
	 * unless they are within a string literal and a string literal starts and
	 * terminates with a single quote '.
	 * 
	...
	/**
	 * Return true if the specified suffix can be found at the supplied position in
	 ...
	 * @return the position of the correct matching nextSuffix or -1 if none can be
	 *         found
	 */
	 ...
	 * @param context          a context for influencing this expression parsing
	 *                         routine (optional)
